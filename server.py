...
def _start_daily_schedule():
    ...
    改良版：記錄「最後執行日期」，只要當天 22:00~23:59 內有任何時機都能補跑。
    解決 Render 重啟導致錯過排程的問題。
    """
    import time as _time

    _last_run_date = {"date": ""}  # 記錄最後執行日期
    ...
    def _scheduler():
        ...
        while True:
            try:
                ...
                # 觸發條件：台灣時間 22:00~23:59，且今天還沒跑過
                if tw_hour >= 22 and _last_run_date["date"] != tw_date:
                    print(f"[排程] ⏰ 觸發每日分析 {tw_dt.strftime('%Y/%m/%d %H:%M')} (TW)")
                    _last_run_date["date"] = tw_date
                    _run_auto_analysis(max_stocks=0, top_n=20, model_ver='v2')
                    print(f"[排程] ✅ 分析完成")
                    _time.sleep(120)  # 完成後休息 2 分鐘
                else:
                    _time.sleep(60)   # 每分鐘檢查一次（比 30 秒更省資源）
            except Exception as e:
                print(f"[排程] ❌ 錯誤: {e}")
                _time.sleep(60)
