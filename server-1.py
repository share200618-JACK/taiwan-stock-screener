...
def set_finmind_token_api():
    ...
    try:
        r = SESSION.get(FINMIND_URL, params={
            "dataset":"TaiwanStockInfo", "data_id":"2330",
        }, headers={"Authorization": f"Bearer {token}"}, timeout=8)
        d = r.json()
        if d.get("status") != 200:
            return jsonify({"error": f"Token 無效或已過期：{d.get('msg','')}"}), 400
    except Exception as e:
        return jsonify({"error": f"驗證失敗：{str(e)}"}), 400
    _save_finmind_token(token)
    _finmind_cache.clear()   # 清除快取，重新用新 token 抓資料
    return jsonify({"ok": True, "msg": "FinMind Token 已儲存並驗證成功！"})
