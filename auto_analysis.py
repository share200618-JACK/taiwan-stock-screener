...
TELEGRAM_TOKEN   = "你的Bot_Token"   # 填入你的 Bot Token
TELEGRAM_CHAT_ID = "2036351531"      # 你的 Chat ID
...
def send_telegram(message):
    try:
        ...
        if not ok: print(f"  Telegram 失敗: {r.text[:100]}")
        return ok
...
def run():
    ...
    send_telegram(
        f"🤖 <b>台股自動分析 v2.0 開始</b>\n"
        f"⏰ {now_str}\n"
        f"改進：2年資料 + 籌碼 + 大盤相對強弱\n"
        f"預測區間：{PREDICT_DAYS} 交易日（約 3 週）\n"
        f"正在分析中，請稍候約 10~15 分鐘..."
    )
    ...
    send_telegram(
        ...
        f"⏰ {now_str}\n"
        f"分析股票：{len(stocks)} 支\n"
        f"有效推薦：{len(results)} 支\n"
        f"平均模型準確率：<b>{avg_acc}%</b>\n"
        f"市場偏多：{bullish} 支 / 偏空：{bearish} 支\n"
        f"預測區間：約 {PREDICT_DAYS//5} 週後漲跌\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"以下依「綜合評分」排序 👇"
    )
    ...
    for rank, r in enumerate(top, 1):
        ...
        prec  = r["precision"]
        price = r["price"]
        pct_s = f"+{r['pct']}%" if r["pct"] >= 0 else f"{r['pct']}%"

        if prob >= 72:   signal = "🔥🔥 極強看漲"
        elif prob >= 65: signal = "🔥 強烈看漲"
        elif prob >= 58: signal = "📈 看漲"
        elif prob >= 52: signal = "↗️ 略偏看漲"
        elif prob >= 45: signal = "↘️ 略偏看跌"
        else:            signal = "📉 看跌"
        ...
        # 籌碼資訊
        inst_str = ""
        if r.get("inst_foreign") or r.get("inst_trust"):
            f_n = r.get("inst_foreign", 0)
            t_n = r.get("inst_trust",   0)
            f_s = f"+{f_n}" if f_n >= 0 else str(f_n)
            t_s = f"+{t_n}" if t_n >= 0 else str(t_n)
            inst_str = f"外資：{f_s}張 / 投信：{t_s}張\n"

        msg = (
            f"<b>#{rank}  {r['name']}（{r['code']}）</b>\n"
            f"━━━━━━━━━━━━━━\n"
            f"訊號：{signal}\n"
            f"上漲機率：<b>{prob}%</b>\n"
            f"信心度：{bar} {conf}%\n"
            f"模型準確率：{acc}%  精確率：{prec}%\n"
            f"訓練資料：{r.get('data_years',0)} 年\n"
            f"現價：{price} 元（今日 {pct_s}）\n"
            f"成交量：{r['vol']:,} 張\n"
            f"{inst_str}"
        )
        send_telegram(msg)
